...
import 'notus_quill_converter.dart';

class HTMLDeltaConverter extends Codec<Delta, String> {
  const HTMLDeltaConverter();

  @override
  Converter<String, Delta> get decoder => _HTMLDeltaDecoder();
...
class _DeltaHTMLEncoder extends Converter<Delta, String> {
  ...
  String convert(Delta input) {
    ...
    void _handleBlock(NotusAttribute<String> blockStyle) {
      ...
      } else if (blockStyle == NotusAttribute.bq || blockStyle == NotusAttribute.code) {
...
//      container = getContainer(op.attributes);
  ...
  void _writeEmbedTag(StringBuffer buffer, NotusAttribute<Map<String, dynamic>> embed, {bool close = false}) {
    if (embed.value[DeltaKeys.type] == DeltaKeys.image) {
      ...
      buffer.write("<${HtmlKeys.horizontalRule}${buildContainer(DeltaKeys.embed)} />");
    }
  }
  ...
  void _writeLinkTag(StringBuffer buffer, NotusAttribute<String> link, {bool close = false}) {
    if (close) {
      ...
      buffer.write('<${HtmlKeys.anchor} ${HtmlKeys.anchorHref}=\"${link.value}\"${buildContainer(DeltaKeys.a)}>');
    }
  }
  ...
  void _writeHeadingTag(StringBuffer buffer, NotusAttribute<int> heading, {bool close = false}) {
    ...
    if (close) {
      ...
      buffer.write('<${HtmlKeys.heading}$level${buildContainer(DeltaKeys.heading)}>');
    }
  }
...
class _HTMLDeltaDecoder extends Converter<String, Delta> {
  final NotusQuillCodec _codec = const NotusQuillCodec();
    ...
    for (final elem in elemStack) {
      switch (elem.localName) {
        ...
        case HtmlKeys.blockquote:
          ...
          break;
        case HtmlKeys.horizontalRule:
          deltaAttributeInline[DeltaKeys.embed] = {DeltaKeys.type: DeltaKeys.hr};
          break;
        case HtmlKeys.list:
        ...
        case HtmlKeys.h3:
          ...
          break;
        case HtmlKeys.anchor:
          deltaAttributeInline[DeltaKeys.a] = elem.attributes[HtmlKeys.anchorHref];
          break;
        case HtmlKeys.bold:
        case HtmlKeys.italic:
  ...
  List<Map<String, dynamic>> toDeltaFormatList(Element element) {
    ...
    void insert(idx, String text, elemStack) {
      ...
      void insertText(txt) {
        ...
        if (attrLine.isNotEmpty && (txt.isNotEmpty || attrInline.containsKey("embed"))) {
          deltaFormatList.insert(shiftIdx(), {
            DeltaKeys.insert: "\n",
    ...
    while (deltaFormatList.map((e) => e is Element).contains(true)) {
      for (int i = 0; i < deltaFormatList.length; i++) {
        ...
        for (var j = 0; j < elem.children.length; j++) {
          ...
          if ((elem.localName != HtmlKeys.orderedList && elem.localName != HtmlKeys.unorderedList) ||
              intervalText != '\n') {
  ...
  Delta convert(String inputHTML) {
    ...
    void addPlainTextToDeltaList(text) {
      if (text.isNotEmpty && text != String.fromCharCode(8203)) {
        if (deltaFormatList.isNotEmpty &&
            deltaFormatList[deltaFormatList.length - 1][DeltaKeys.attributes] == null) {
    ...
    removeRedundantNewLine(deltaList) {
      for (int i = 1; i < deltaList.length; i++) {
        ...
        if (text.startsWith("\n")) {
          ...
          if (attr != null && (attr.containsKey("block") || attr.containsKey("heading"))) {
            if (text == "\n") {
    ...
    avoidConcatinatingPlainTextAndLineAttributes(deltaList) {
      ...
      for (int i = 1; i < deltaList.length - 1; i++) {
        ...
        Map<String, dynamic> current = deltaList[i];
        Map<String, dynamic> prev = deltaList[i - 1];
        if (isIncludeLineAttributes(next) && !prev[DeltaKeys.insert].endsWith('\n')) {
    ...
    insertNewlineAfterConsecutiveAnchorTagWithImage(deltaList) {
      for (int i = 0; i < deltaList.length; i++) {
        ...
        Map<String, dynamic> next = (i == deltaList.length - 1) ? null : deltaList[i + 1];
        Map<String, dynamic> current = deltaList[i];
        Map<String, dynamic> nextAttr = (i == deltaList.length - 1) ? null : next[DeltaKeys.attributes];
        Map<String, dynamic> currentAttr = current[DeltaKeys.attributes];
        Map<String, dynamic> prevAttr = (i == 0) ? null : prev[DeltaKeys.attributes];
        if (currentAttr == null ||
    ...
    final notusDelta = Delta.fromJson(deltaFormatList);
    final delta = _codec.encode(notusDelta);
    return delta;
  }
}
/*
...
class HTMLConverter extends Codec<Delta, String> {
  const HTMLConverter();

  @override
  Converter<String, Delta> get decoder => _HTMLDecoder();

  @override
  Converter<Delta, String> get encoder => _HTMLEncoder();
}
...
class _HTMLEncoder extends Converter<Delta, String> {
  static final kSimpleBlocks = <Attribute, String>{
    Attribute.blockQuote: HtmlKeys.blockquote,
    Attribute.ul: HtmlKeys.unorderedList,
    Attribute.ol: HtmlKeys.orderedList,
  };
  Map<String, dynamic> container;
  ...
  String convert(Delta input) {
    ...
    void _handleBlock(Attribute<String> blockStyle) {
      ...
      if (blockStyle == null) {
        buffer.write(currentBlockLines.join('\n'));
      } else if (blockStyle.key == Attribute.blockQuote.key || blockStyle.key == Attribute.codeBlock.key) {
        _writeBlockTag(buffer, blockStyle, start: true);
        buffer.write(currentBlockLines.join('\n'));
        _writeBlockTag(buffer, blockStyle, close: true);
      } else {
    ...
    void _handleLine(Map<String, dynamic> attributes) {
      final style = Style.fromJson(attributes);
      final lineBlock = style.get(Attribute.blockQuote.key);
      if (lineBlock == currentBlockStyle) {
        currentBlockLines.add(_writeLine(lineBuffer.toString(), style));
      } else {
        _handleBlock(currentBlockStyle);
  ...
  String _writeLine(String text, Style style) {
    ...
    if (style.containsKey(Attribute.header.key)) {
      _writeAttribute(buffer, style.get(Attribute.header.key));
    }

    // Write the text itself
    buffer.write(text);
    if (style.containsKey(Attribute.header.key)) {
      _writeAttribute(buffer, style.get(Attribute.header.key), close: true);
    }
    return buffer.toString();
  }
  ...
  Style _writeInline(StringBuffer buffer, String text, Style style, Style currentStyle) {
    // First close any current styles if needed
    for (final value in currentStyle.values.toList().reversed) {
      if (value.scope == AttributeScope.IGNORE) continue;
      if (style.containsSame(value)) continue;
      final padding = _trimRight(buffer);
      _writeAttribute(buffer, value, close: true);
      if (padding.isNotEmpty) buffer.write(padding);
  ...
  void _writeAttribute(StringBuffer buffer, Attribute attribute, {bool close = false}) {
    if (attribute == Attribute.bold) {
      ...
      _writeHeadingTag(buffer, attribute, close: close);
    } else if (attribute.key == Attribute.blockQuote.key) {
      _writeBlockTag(buffer, attribute, close: close);
    } else {
      throw ArgumentError('Cannot handle $attribute');
    }
  ...
  void _writeEmbedTag(StringBuffer buffer, Attribute<Map<String, dynamic>> embed, {bool close = false}) {
    if (embed.value[DeltaKeys.type] == DeltaKeys.image) {
      if (close) {
        return;
      }
  ...
  void _writeLinkTag(StringBuffer buffer, Attribute<String> link, {bool close = false}) {
    if (close) {
      buffer.write('</${HtmlKeys.anchor}>');
    } else {
      buffer.write('<${HtmlKeys.anchor} ${HtmlKeys.anchorHref}=\"${link.value}\"${buildContainer(DeltaKeys.a)}>');
  ...
  void _writeHeadingTag(StringBuffer buffer, Attribute<int> heading, {bool close = false}) {
  ...
  void _writeBlockTag(StringBuffer buffer, Attribute<String> block, {bool close = false, bool start = false}) {
    if (block.key == Attribute.codeBlock.key) {
      if (start) {
        buffer.write('<${HtmlKeys.preformatted}${buildContainer(block.key)}>');
      } else if (close) {
        buffer.write('</${HtmlKeys.preformatted}>');
...
bool isAllowedHTML(Element elem) {
  ...
  final Queue queue = Queue<Element>()..add(elem);
  while (queue.isNotEmpty) {
    final Element target = queue.removeFirst();
    if (!_allowedHTMLTag.contains(target.localName)) {
      return false;
    }
    queue.addAll(target.children);
...
class _HTMLDecoder extends Converter<String, Delta> {
  Map<String, Map<String, dynamic>> toDeltaAttribute(Queue<Element> elemStack) {
  ...
  List<Map<String, dynamic>> toDeltaFormatList(Element element) {
    ...
    deltaFormatList.add(element);
    final map = <Element, Queue<Element>>{element: elemStack};
    while (deltaFormatList.map((e) => e is Element).contains(true)) {
      for (var i = 0; i < deltaFormatList.length; i++) {
        final val = deltaFormatList[i];
        if ((val is Element) == false) continue;
        deltaFormatList.removeAt(i);
        final Element elem = val;
        final currentElemStack = map[elem];
        var cursor = 0;
        final htmlString = elem.innerHtml;
        final originalLength = deltaFormatList.length;
        ...
        for (var j = 0; j < elem.children.length; j++) {
          ...
          final newElemStack = Queue<Element>.from(currentElemStack)..addLast(child);
          if (child.children.isNotEmpty) {
            deltaFormatList.insert(i + shiftIdx(), child);
            map[child] = newElemStack;
          } else {
  ...
  Delta convert(String inputHTML) {
    ...
    var cursor = 0;
    for (final firstLayerTag in rootHTML.children) {
    ...
    void avoidConcatinatingPlainTextAndLineAttributes(deltaList) {
      ...
      for (var i = 1; i < deltaList.length - 1; i++) {
        ...
        final current = deltaList[i];
        final Map<String, dynamic> prev = deltaList[i - 1];
        if (isIncludeLineAttributes(next) && !prev[DeltaKeys.insert].endsWith('\n')) {
          current[DeltaKeys.insert] = '\n${current[DeltaKeys.insert]}';
        }
      }
    ...
    void ensureEndWithNewLine(deltaList) {
      final int lastIndex = deltaList.length - 1;
      final String lastText = deltaList[lastIndex][DeltaKeys.insert];
      if (!lastText.endsWith('\n')) {
        if (deltaList[lastIndex][DeltaKeys.attributes] != null) {
          deltaList.add({DeltaKeys.insert: '\n'});
        } else {
    ...
    void removeZeroWidthSpaceFromNonEmbed(deltaList) {
      for (final Map<String, dynamic> deltaFormat in deltaList) {
        final Map<String, dynamic> attr = deltaFormat[DeltaKeys.attributes];
        if (attr != null && attr.containsKey(DeltaKeys.embed)) {
          continue;
        }
        String text = deltaFormat[DeltaKeys.insert];
    ...
    void insertNewlineAfterConsecutiveAnchorTagWithImage(deltaList) {
      for (var i = 0; i < deltaList.length; i++) {
        final Map<String, dynamic> prev = (i == 0) ? null : deltaList[i - 1];
        final Map<String, dynamic> next = (i == deltaList.length - 1) ? null : deltaList[i + 1];
        final Map<String, dynamic> current = deltaList[i];
        final Map<String, dynamic> nextAttr = (i == deltaList.length - 1) ? null : next[DeltaKeys.attributes];
        final Map<String, dynamic> currentAttr = current[DeltaKeys.attributes];
        final Map<String, dynamic> prevAttr = (i == 0) ? null : prev[DeltaKeys.attributes];
        if (currentAttr == null ||
            !currentAttr.containsKey(DeltaKeys.a) ||
            !currentAttr.containsKey(DeltaKeys.embed) ||
            currentAttr[DeltaKeys.embed][DeltaKeys.type] != DeltaKeys.image) {
